...
| 33c | `.github/skills/calibration-report/SKILL.md` | Skill: `/calibration-report` — longitudinal verdict accuracy from the verdict archive: Brier score per band, confusion matrix, per-layer signal fidelity, and rolling-window drift. Wraps `.assert-iq/analysis/calibration.py` (the single implementation) and interprets its output against the maturity tier. Pairs with `/measure-qi-impact` for the quarterly review. |
...
| 47q | `.assert-iq/tests/_qi/automated/unit-verdict-recorder-stdlib.py` | Pins the stdlib-only contract for `verdict-recorder.py`: the module must import with PyYAML absent (a module-scope `import yaml` once killed `VerdictRecorder` and `compute_memory_hash` too), the verdicts gate must still resolve from `config.yaml` via the fallback reader rather than silently reading as "disabled", and that reader must omit what it cannot represent instead of inventing values. |
| 47r | `.assert-iq/tests/_qi/automated/unit-memory-hash-portability.py` | The same memory store must hash identically on every platform, because `memory_version` is the reproducibility key. Covers host path-collation, CRLF vs LF checkouts, macOS `.DS_Store` / AppleDouble and Windows `Thumbs.db` / `desktop.ini`, length-prefixed `(path, content)` framing, and NFC filename normalization — while proving the digest still responds to real edits. |
| 47s | `.assert-iq/tests/_qi/automated/unit-dream-safety.py` | Proves the dream safety procedure runs rather than merely being documented. Most load-bearing assertion: restoring a snapshot reproduces the exact `memory_version` recorded for that cycle — step 1 of the reproducibility contract, which could never have worked before. Also covers tier-gated blocking, cycle-id uniqueness within one second, snapshot retention, and the regression divergence gate. |
...
| 69 | `.assert-iq/kiro-harness.md` | **Kiro** The verified schema contract for the third harness — derived from the shipped Kiro binary, not its public docs, which are wrong about the agent file format, about `allowedTools`/`toolsSettings`, and about the hook schema. Read before changing anything under `.kiro/`. Records what is unverified as well as what is confirmed. |
| 70 | `.kiro/steering/00-assert-iq.md` | **Kiro** Hand-authored entrypoint. Kiro has no root-level entrypoint file, so the counterpart to `CLAUDE.md` IS a steering file with `inclusion: always`. |
| 71-76 | `.kiro/steering/qi-*.md` | **Kiro — GENERATED, do not hand-edit.** Six steering files rendered from `.github/instructions/*.instructions.md` by `scripts/sync-kiro.{sh,ps1}`, mapping Copilot's `applyTo` to Kiro's `inclusion` / `fileMatchPattern`. Brace groups are expanded rather than passed through, because Kiro's brace support is unverified. Freshness enforced by check P7 in `e2e-agent-parity.sh`. |
| 77 | `.kiro/agents/assert-iq.md` | **Kiro** Hand-authored lead orchestrator. Routing coverage enforced by check P3. |
| 78 | `.kiro/agents/assert-iq-plan.md` | **Kiro** Hand-authored read-only planning sibling. Read-only is enforced twice — `tools` omits write/shell AND `excludedTools` names them — so widening `tools` later cannot silently hand a planning agent an editor. |
| 79-86 | `.kiro/agents/<specialist>.md` | **Kiro — GENERATED, do not hand-edit.** The same 8 specialists rendered from `.claude/agents/specialists/*.md` by `scripts/sync-kiro.{sh,ps1}`, mapping Claude tool names to Kiro capability **tags** (tags, not names: Kiro's own guidance is that tags survive tool renames). Each declares both `resources` globs — without them a Kiro custom agent runs with **no QI rulebook**, silently, because invalid resource entries are dropped without error. |
| 87 | `.kiro/skills` | **Kiro** Symlink → `../.github/skills/` so Kiro discovers the same 31 skills as Copilot and Claude. Falls back to a copy where symlinks are unavailable; re-run the installer after editing a skill or the copy goes stale. |
| 88 | `.kiro/settings/mcp.json` | **Kiro** Hand-maintained MCP config. Not a copy of `.vscode/mcp.json`: the key is `mcpServers` not `servers`, transport is inferred from `command` vs `url` (no `type`), and credentials are `${ENV_VAR}` because Kiro has no `${input:...}` prompt. All 20 servers ship `disabled: true`. |
| 89 | `.kiro/settings/MCP.md` | **Kiro** Companion doc. Documents the trap: exporting the variable is not enough — Kiro expands `${VAR}` only for names approved in `kiroAgent.mcpApprovedEnvVars`, and an unapproved one stays literal, so the server starts and fails to authenticate with no hint why. |
| 90 | `.kiro/hooks/assert-iq-dreaming.json` | **Kiro — RENDERED at install time** from `.assert-iq/dreaming/kiro-hooks.{posix,windows}.template.json`. Dreaming's `SessionStart` gate and `Stop` recorder in Kiro's v1 hook schema. Excluded from git. |
| 91-92 | `.assert-iq/dreaming/kiro-hooks.{posix,windows}.template.json` | **Kiro** Committed source for the above. A third hook shape, not a variant of the other two: a flat `hooks[]` array with PascalCase triggers, no matcher-group wrapper and no `shell` field. Prefers Kiro's `${WORKSPACE_ROOT}` substitution with the installer-baked `__PACK_ROOT__` as fallback, and names its interpreter explicitly because Kiro spawns with `shell:true` (`/bin/sh` on POSIX, `cmd.exe` on Windows). |
| 93-94 | `scripts/sync-kiro.{sh,ps1}` | **Kiro** Generators for rows 71-76 and 79-86. `--check` verifies freshness (P7); `--print-map` emits the tool map so the two implementations cannot diverge (P8). Both must produce byte-identical output. |
| 95 | `.assert-iq/tests/_qi/automated/unit-kiro-schema.py` | **Kiro** Validates all four Kiro surfaces against the schema the binary enforces — steering inclusion modes, agent tags and resources, hook triggers and actions, MCP shape — plus frontmatter on every skill. |
| — | `.assert-iq/analysis/dream-safety.py` | The pre/post-dream safety procedure `qi-foundation.instructions.md` requires: sanity checks, a memory snapshot to `dreaming/.snapshots/`, an append-only cycle record in `dreaming/provenance.json`, and the post-dream golden-corpus regression gate. Blocks at `higher` tier, warns below it. Invoked by `/dream` (`pre` before consolidating, `post` after). Before this existed the procedure was documented but unwired — nothing wrote provenance and `.snapshots/` stayed empty, so the reproducibility contract's `tar xzf` step could never work. |
...
**Skill count**: 31 (in `.github/skills/`)
