...
#   --aggregator-quickstart  Write .assert-iq/aggregator-quickstart.yaml
...
for arg in "$@"; do
  case "$arg" in
    ...
    --aggregator-quickstart) AGG_QUICKSTART=1 ;;
    --help|-h)
...
# =============================================================================
# --aggregator-quickstart short-circuit
# =============================================================================
#
...
if [[ $AGG_QUICKSTART -eq 1 ]]; then
  echo "=== Assert.IQ aggregator quickstart ==="
  SAMPLES_DIR="$SOURCE/mcp/qi-signal-aggregator/samples"
  if [[ ! -d "$SAMPLES_DIR/fixtures" ]]; then
    echo "ERROR: bundled samples not found at $SAMPLES_DIR/fixtures" >&2
    echo "       Are you running this from inside the agent pack?" >&2
    exit 2
  fi

  mkdir -p "$WORKSPACE/.assert-iq"
  QS_FILE="$WORKSPACE/.assert-iq/aggregator-quickstart.yaml"

  # Decide which adapters to enable based on which fixture dirs exist.
  CHANGE_LIST="github"
  OUTCOME_LIST="sentry, jira"
  EXTRA_SETTINGS=""
  if [[ -d "$SAMPLES_DIR/fixtures/ado_repos" ]]; then
    CHANGE_LIST="$CHANGE_LIST, ado_repos"
    EXTRA_SETTINGS+="    ado_repos:
      fixture_dir: \"$SAMPLES_DIR/fixtures/ado_repos\"
"
  fi
  if [[ -d "$SAMPLES_DIR/fixtures/ado_boards" ]]; then
    OUTCOME_LIST="$OUTCOME_LIST, ado_boards"
    EXTRA_SETTINGS+="    ado_boards:
      fixture_dir: \"$SAMPLES_DIR/fixtures/ado_boards\"
"
  fi

  cat > "$QS_FILE" <<YAML
# qi-signal-aggregator quickstart config — generated by scripts/bootstrap.sh
#   --aggregator-quickstart on $(date -u +%Y-%m-%dT%H:%M:%SZ)
#
# Every adapter is wired to the bundled sample fixtures in the agent pack.
# No network, no secrets. Re-run --aggregator-quickstart any time to refresh
# paths after upgrading the pack.
#
# Try it:
#   qi-signal-aggregator --config $QS_FILE health
#   qi-signal-aggregator --config $QS_FILE demo --id pr-099-red
...
signal_aggregator:
  ...
  adapter_settings:
    github:
      fixture_dir: "$SAMPLES_DIR/fixtures/github"
    coverage_xml:
      path: "$SAMPLES_DIR/fixtures/coverage/coverage.xml"
    qi_traceability_scan:
      scan_root: "$SAMPLES_DIR/fixtures/repo"
      includes: ["**/*.py"]
      excludes: ["**/tests/**"]
    junit_glob:
      glob: "$SAMPLES_DIR/fixtures/junit/*.xml"
    sentry:
      fixture_dir: "$SAMPLES_DIR/fixtures/sentry"
    jira:
      fixture_dir: "$SAMPLES_DIR/fixtures/jira"
$EXTRA_SETTINGS
YAML

  echo "[ok] wrote $QS_FILE"

  # Locate the binary on common install paths and PATH.
  BIN=""
  for cand in "$HOME/.local/bin/qi-signal-aggregator" \
              "/usr/local/bin/qi-signal-aggregator" \
              "$(command -v qi-signal-aggregator 2>/dev/null || true)"; do
    if [[ -n "$cand" && -x "$cand" ]]; then BIN="$cand"; break; fi
  done

  echo ""
  if [[ -z "$BIN" ]]; then
    echo "Binary not found on standard paths. Install it first:"
    echo "  bash $SOURCE/install.sh"
    echo ""
    echo "Then try:"
    echo "  qi-signal-aggregator --config $QS_FILE health"
    echo "  qi-signal-aggregator --config $QS_FILE demo --id pr-099-red"
  else
    echo "Binary found: $BIN"
    echo "Try it now:"
    echo "  $BIN --config $QS_FILE health"
    echo "  $BIN --config $QS_FILE demo --id pr-099-red"
    echo ""
    echo "To wire this config into VS Code Copilot, edit .vscode/mcp.json so the"
    echo "qi-signal-aggregator server's --config arg points at:"
    echo "  $QS_FILE"
    echo "then reload the window (Cmd/Ctrl+Shift+P -> Developer: Reload Window)."
  fi
  exit 0
fi
