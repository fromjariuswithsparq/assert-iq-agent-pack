...
render_steering() {
  ...
  printf -- '     by the Kiro sync (applyTo -> inclusion/fileMatchPattern).\n'
  printf -- '     To change this steering file, edit that instruction source and\n'
  printf -- '     re-run the sync FROM THE PACK CHECKOUT. scripts/ is pack-only and\n'
  printf -- '     is deliberately not installed, so `scripts/sync-kiro.sh` (or the\n'
  printf -- '     .ps1) does not resolve in an installed workspace. That is\n'
  printf -- '     expected -- it is not a broken install.\n'
...
render_agent() {
  ...
  printf -- '     by the Kiro sync (tool names mapped Claude -> Kiro tags).\n'
  printf -- '     To change this agent, edit that Claude source and re-run the\n'
  printf -- '     sync FROM THE PACK CHECKOUT. scripts/ is pack-only and is\n'
  printf -- '     deliberately not installed, so `scripts/sync-kiro.sh` (or the\n'
  printf -- '     .ps1) does not resolve in an installed workspace. That is\n'
  printf -- '     expected -- it is not a broken install.\n'
  printf -- '     Staleness is enforced by check P7 in\n'
  printf -- '     .assert-iq/tests/_qi/automated/e2e-agent-parity.sh\n'
