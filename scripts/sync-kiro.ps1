...
function Render-Steering {
    ...
    [void]$sb.Append("     by the Kiro sync (applyTo -> inclusion/fileMatchPattern).`n")
    [void]$sb.Append("     To change this steering file, edit that instruction source and`n")
    [void]$sb.Append("     re-run the sync FROM THE PACK CHECKOUT. scripts/ is pack-only and`n")
    [void]$sb.Append("     is deliberately not installed, so ``scripts/sync-kiro.sh`` (or the`n")
    [void]$sb.Append("     .ps1) does not resolve in an installed workspace. That is`n")
    [void]$sb.Append("     expected -- it is not a broken install.`n")
...
function Render-Agent {
    ...
    [void]$sb.Append("     by the Kiro sync (tool names mapped Claude -> Kiro tags).`n")
    [void]$sb.Append("     To change this agent, edit that Claude source and re-run the`n")
    [void]$sb.Append("     sync FROM THE PACK CHECKOUT. scripts/ is pack-only and is`n")
    [void]$sb.Append("     deliberately not installed, so ``scripts/sync-kiro.sh`` (or the`n")
    [void]$sb.Append("     .ps1) does not resolve in an installed workspace. That is`n")
    [void]$sb.Append("     expected -- it is not a broken install.`n")
    [void]$sb.Append("     Staleness is enforced by check P7 in`n")
    [void]$sb.Append("     .assert-iq/tests/_qi/automated/e2e-agent-parity.sh`n")
