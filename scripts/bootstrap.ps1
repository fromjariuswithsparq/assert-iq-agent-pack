...
#   -AggregatorQuickstart   Write .assert-iq/aggregator-quickstart.yaml
#                           wired to the bundled sample fixtures, so the
#                           qi-signal-aggregator MCP server returns real
#                           verdicts immediately. No network, no secrets.
#
...
# =============================================================================
# -AggregatorQuickstart short-circuit
# =============================================================================
#
# Generates .assert-iq/aggregator-quickstart.yaml pointing at the bundled
# sample fixtures (absolute paths) so the qi-signal-aggregator MCP server
# returns real verdicts immediately, with no network calls and no secrets.

if ($AggregatorQuickstart) {
    Write-Host '=== Assert.IQ aggregator quickstart ==='
    $samplesDir = Join-Path $Source 'mcp/qi-signal-aggregator/samples'
    if (-not (Test-Path -LiteralPath (Join-Path $samplesDir 'fixtures'))) {
        Write-Error "bundled samples not found at $samplesDir/fixtures. Are you running this from inside the agent pack?"
        exit 2
    }

    $assertIqDir = Join-Path $Workspace '.assert-iq'
    New-Item -ItemType Directory -Force -Path $assertIqDir | Out-Null
    $qsFile = Join-Path $assertIqDir 'aggregator-quickstart.yaml'

    $changeList = 'github'
    $outcomeList = 'sentry, jira'
    $extra = ''
    if (Test-Path -LiteralPath (Join-Path $samplesDir 'fixtures/ado_repos')) {
        $changeList = "$changeList, ado_repos"
        $extra += "    ado_repos:`n      fixture_dir: `"$samplesDir/fixtures/ado_repos`"`n"
    }
    if (Test-Path -LiteralPath (Join-Path $samplesDir 'fixtures/ado_boards')) {
        $outcomeList = "$outcomeList, ado_boards"
        $extra += "    ado_boards:`n      fixture_dir: `"$samplesDir/fixtures/ado_boards`"`n"
    }

    $timestamp = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
    $yaml = @"
# qi-signal-aggregator quickstart config — generated by scripts/bootstrap.ps1
#   -AggregatorQuickstart on $timestamp
#
# Every adapter is wired to the bundled sample fixtures in the agent pack.
# No network, no secrets. Re-run -AggregatorQuickstart any time to refresh
# paths after upgrading the pack.
#
# Try it:
#   qi-signal-aggregator --config $qsFile health
#   qi-signal-aggregator --config $qsFile demo --id pr-099-red
...
signal_aggregator:
  ...
  adapter_settings:
    github:
      fixture_dir: `"$samplesDir/fixtures/github`"
    coverage_xml:
      path: `"$samplesDir/fixtures/coverage/coverage.xml`"
    qi_traceability_scan:
      scan_root: `"$samplesDir/fixtures/repo`"
      includes: [`"**/*.py`"]
      excludes: [`"**/tests/**`"]
    junit_glob:
      glob: `"$samplesDir/fixtures/junit/*.xml`"
    sentry:
      fixture_dir: `"$samplesDir/fixtures/sentry`"
    jira:
      fixture_dir: `"$samplesDir/fixtures/jira`"
$extra
"@
    Set-Content -LiteralPath $qsFile -Value $yaml -Encoding UTF8
    Write-Host "[ok] wrote $qsFile"

    $bin = $null
    $candidates = @(
        (Join-Path $env:LOCALAPPDATA 'qi-signal-aggregator\bin\qi-signal-aggregator.exe'),
        (Join-Path $HOME '.local/bin/qi-signal-aggregator.exe'),
        (Join-Path $HOME '.local/bin/qi-signal-aggregator')
    )
    foreach ($c in $candidates) {
        if (Test-Path -LiteralPath $c) { $bin = $c; break }
    }
    if (-not $bin) {
        $cmd = Get-Command qi-signal-aggregator -ErrorAction SilentlyContinue
        if ($cmd) { $bin = $cmd.Source }
    }

    Write-Host ''
    if (-not $bin) {
        Write-Host 'Binary not found on standard paths. Install it first:'
        Write-Host "  pwsh -File $Source\install.ps1"
        Write-Host ''
        Write-Host 'Then try:'
        Write-Host "  qi-signal-aggregator --config $qsFile health"
        Write-Host "  qi-signal-aggregator --config $qsFile demo --id pr-099-red"
    } else {
        Write-Host "Binary found: $bin"
        Write-Host 'Try it now:'
        Write-Host "  $bin --config $qsFile health"
        Write-Host "  $bin --config $qsFile demo --id pr-099-red"
        Write-Host ''
        Write-Host 'To wire this config into VS Code Copilot, edit .vscode/mcp.json so the'
        Write-Host "qi-signal-aggregator server's --config arg points at:"
        Write-Host "  $qsFile"
