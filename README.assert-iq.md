...
|---|---|
| **Claude Code** (`/plugin install`) | Yes — append `@v0.7.0-rc.5`. |
...
3. Paste the `owner/repo` shorthand — **no `@ref`, no `https://` prefix**:
   ...
   > found*; appending `@v0.7.0-rc.5` returns *not a valid plugin source*.
...
1. In Claude Code, run the plugin install slash command. Claude Code
   ...
   ```
   /plugin install fromjariuswithsparq/assert-iq-agent-pack@v0.7.0-rc.5
   ```

   Drop the `@v0.7.0-rc.5` suffix to install from the default branch.
...
```bash
git clone --depth 1 --branch v0.7.0-rc.5 \
